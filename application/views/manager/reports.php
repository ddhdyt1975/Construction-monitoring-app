                        ...
                        <div class="box-body" >
                            ...
                            <table width="90%" align="center" BORDER="0" CELLSPACING="0" id ='tbl1'>
                               <thead>
                                    <tr>
                                        <th align="center">
                                            <p style = "font-size:50px;"><!-- <img src ="<?php echo $user_info->path?>" width = "100" heigth = "100"/> --> <u>obraOne</u></p>
                                        </th>
                                    </tr>
                                    <tr>
                                        <th colspan="7">
...
<script>
        ...
        success: function(data) {
           if (data.length >0){
            for(i=0; i<data.length;i++){
                    $('#tbl2 ul').append('<li> Task '+data[i]['projtsk_name']+' updated to '+data[i]['updated_percent']+'%.</li><div id ="nested'+data[i]['updated_task_id']+'">');
            ...
            if (data.length >0){
                for(i=0; i<data.length;i++){
                    if ((data[i]['prjtsk_status'] == "Unresolved") || (data[i]['prjtsk_status'] == "Progress") || (data[i]['prjtsk_status'] == "Critical") || (data[i]['prjtsk_status'] == "Open")) {
                        $('#ptsk ul').append('<li>Task '+data[i]['projtsk_name']+' ('+ data[i]['projdtsk_percent']+'%) - '+data[i]['prjtsk_comment']+'. - <b>'+data[i]['prjtsk_status']+' ('+ data[i]['prjtsk_comment_date']+')</b></li>');
                    }
                };
                $('#ptsk ').append('</ul><ol>');
            }
    ...
    function MUDetails(){
        ...
        dataType: "JSON",
            success: function(data) {
                if (data.length >0){
                    for(i=0; i<data.length;i++){
                        $('#tbl4 ul').append('<li >'+data[i]['mu_quantity'] +' '+ data[i]['material_unit']+'(s) of '+data[i]['material_name']+' is used from Warehouse.</li>');
                    };
    ...
    function MUDetails2(){
        ...
        dataType: "JSON",
            success: function(data) {
                if (data.length >0){
                    for(i=0; i<data.length;i++){
                        $('#tbl4 ul').append('<li >'+data[i]['mu_quantity'] +' '+ data[i]['material_unit']+'(s) of '+data[i]['material_name']+' is used.');//' transfered from '+data[i]['project_title']+'.</li>');
                    };
    ...
    function WRKDetails(){
        ...
        dataType: "JSON",
            success: function(data) {
                 if (data.length >0){
                    data = sortByKey(data, 'woker_type_desc')
                    for(i=0; i<data.length;i++){ 
                        if (data[i]['woker_type_desc']=='Owner Representative'){
                            $('#ii').append('<p><u>Owner\'s Representative:</u> '+ data[i]['pworker_fname']+' '+ data[i]['pworker_lname']+'</p>');
                        }
                        else if (data[i]['woker_type_desc']=='Company Representative'){
                            $('#ii').append('<p><u>Company Representative:</u> '+ data[i]['pworker_fname']+' '+ data[i]['pworker_lname']+'</p>');
                        }
                        else {
                            $('#tbl5 ul').append('<li>'+data[i]['woker_type_desc'] +' - '+ data[i]['pworker_fname']+' '+ data[i]['pworker_lname']+'</li>');
                        }
                    };
                    $('#tbl5 ul').append('</ul><ol>');
                 }
            },
            error: function (jqXHR, textStatus, errorThrown) {
