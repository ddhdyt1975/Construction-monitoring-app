...
class Project2 extends CI_Model {
    ...
    public function get_eu_id($id){
          ...
          $query = $this->db->query('SELECT equipment_used.eu_id, equipment.equipment_name, equipment_used.eu_quantity, supplier.supplier_name, equipment_used.eu_date FROM project INNER JOIN equipment_used ON equipment_used.project_code = project.project_code INNER JOIN equipment ON equipment.equipment_id = equipment_used.equipment_id INNER JOIN supplier ON supplier.supplier_id = equipment.supplier_id WHERE project.project_code = "'.$id.'"');
