...
class Report extends CI_Model {
    ...
    public function getAllProjects($id, $com){
    ...
    function getEUDetails1($project_title, $date, $date2,$date3) {
        $query = $this->db->query('SELECT equipment_used.eu_id, equipment_used.equipment_id, equipment_used.eu_date, equipment_used.eu_quantity, equipment_used.project_code, equipment_used.received_from, DATE_FORMAT(eu_date,"%m/%d/%Y"), equipment.equipment_name FROM equipment_used INNER JOIN equipment ON equipment.equipment_id = equipment_used.equipment_id WHERE equipment_used.project_code ="'.$project_title.'" AND DATE_FORMAT(eu_date,"%m/%d/%Y")  = "'.$date.'/'.$date2.'/'.$date3.'" AND received_from = ""');
        return $query->result();
    }
    ...
    public function getnn_by_id($id){
        $query = $this->db->query('SELECT * FROM `nextdayact` WHERE next_id = "'.$id.'"');
        return $query->row();
    }
    public function upN($where, $data){
        $this->db->update('nextdayact', $data, $where);
        return $this->db->affected_rows();
    }

    public function deletew_by_id($id){
        $this->db->where('projdtsk_id', $id);
        $this->db->delete('worker');
    }

    public function deletenn_by_id($id){
        $this->db->where('next_id', $id);
        $this->db->delete('nextdayact');
    }
}
