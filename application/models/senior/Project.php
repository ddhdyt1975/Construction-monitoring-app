...
class Project extends CI_Model {
    ...
    public function getNA($id){
        $query = $this->db->query('SELECT
                                    ...
                                    nextdayact.next_id,
                                    nextdayact.status,
                                    DATE_FORMAT(postDate,"%h:%i %p") AS time,
    ...
    function getProjDetails($project_title) {
        $query = $this->db->query('SELECT * FROM  city c INNER JOIN province ON c.province_id = province.province_id   INNER JOIN project ON project.city_id = c.city_id  INNER JOIN user ON project.user_id = `user`.user_id INNER JOIN project_details ON  project.project_code = project_details.project_code  WHERE project.project_code  = "'.$project_title.'" ');
        return $query->row();
    }
